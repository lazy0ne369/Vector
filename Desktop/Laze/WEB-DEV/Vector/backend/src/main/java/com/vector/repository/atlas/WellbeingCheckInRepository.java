...
public interface WellbeingCheckInRepository extends JpaRepository<WellbeingCheckIn, Long> {
    ...
    @Query("SELECT w FROM WellbeingCheckIn w WHERE w.user.id = :userId ORDER BY w.checkDate DESC LIMIT :limit")
    List<WellbeingCheckIn> findLatestByUserId(@Param("userId") Long userId, @Param("limit") int limit);
